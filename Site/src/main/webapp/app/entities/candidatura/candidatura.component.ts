...
export class CandidaturaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.candidaturaService
      ...
      .subscribe(
        (res: HttpResponse<ICandidatura[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ICandidatura): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInCandidaturas(): void {
    this.eventSubscriber = this.eventManager.subscribe('candidaturaListModification', () => this.loadPage());
  }

  delete(candidatura: ICandidatura): void {
    const modalRef = this.modalService.open(CandidaturaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ICandidatura[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
